...
locals() # - показывает все локалные переменные со всеми глобальными переменными
globals() # - показывает все глобальные перенные со всеми глобальными переменными

# LEGB - local, enclosed, global, build-in

# Build-in - встроенное пространство имен (все встроенные переменные (print, input, sum, max, min, len, abs, int, str, dict ....))

# Global - глобальные пространство имен (все переменные внутри файла, которые создали мы)
# чтобы узнать, что находится в глобальном пространстве, можно использовать функцию global
